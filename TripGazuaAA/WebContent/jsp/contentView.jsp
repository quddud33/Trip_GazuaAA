	...
	<c:choose>
		...
		<c:when test="${param.contenttypeid == '15' }">
			...
			<table border="2" class="table table-boarded">
				...
				<button style="display: block;margin: auto; margin-bottom: 10px;" onclick="location.href='festvalWish.do?userID=${user.userID}&contentID=${commonInfo.contentid }&contentTypeID=15'">찜하기</button>
	...
	<c:if test="${user ne null }">
		<form action="reviewWrite.do"
			...
			<input type="hidden" name="contentTypeID"
				value="${param.contenttypeid }"> <input type="hidden"
				name="userID" value="${user.userID }"> <input type="hidden"
				name="nickname" value="${user.nickname }"> <span
				class="star-input"> <span class="input"> <input
					type="radio" name="score" id="p1" value="1"><label for="p1">1</label>
					<input type="radio" name="score" id="p2" value="2"><label
					for="p2">2</label> <input type="radio" name="score" id="p3"
					value="3"><label for="p3">3</label> <input type="radio"
					name="score" id="p4" value="4"><label for="p4">4</label> <input
					type="radio" name="score" id="p5" value="5"><label for="p5">5</label>
					<input type="radio" name="score" id="p6" value="6"><label
					for="p6">6</label> <input type="radio" name="score" id="p7"
					value="7"><label for="p7">7</label> <input type="radio"
					name="score" id="p8" value="8"><label for="p8">8</label> <input
					type="radio" name="score" id="p9" value="9"><label for="p9">9</label>
					<input type="radio" name="score" id="p10" value="10"><label
					for="p10">10</label>
			</span> <output for="star-input"> <b>0</b>점</output>
			</span>
		</form>
	</c:if>
	<table border="2" class="table table-boarded table-striped"
		...
		<c:forEach items="${reviewL }" var="reviewL">
			<tr>
				...
				<c:if test="${user.userid eq reviewL.userID }">
					<th><a
						href="reviewDelete.do?num=${reviewL.num }&content=${reviewL.content}&contenttypeid=${param.contenttypeid }&contentid=<%=request.getParameter("contentid")%>">삭제</a>
						<a
						href="reviewUpdateForm.do?num=${reviewL.num }&content=${reviewL.content}&contenttypeid=${param.contenttypeid }&contentid=<%=request.getParameter("contentid")%>">수정</a>
					</th>
				</c:if>
				<c:if test="${user ne null}">
					<th><button
							onclick="location.href='likeSum.do?likeCount=${reviewL.likeCount }&num=${reviewL.num }&content=${reviewL.content}&contenttypeid=${param.contenttypeid }&contentid=<%=request.getParameter("contentid")%>'">좋아요</button></th>
					<th><button
							onclick="location.href='likeMinus.do?likeCount=${reviewL.likeCount }&num=${reviewL.num }&content=${reviewL.content}&contenttypeid=${param.contenttypeid }&contentid=<%=request.getParameter("contentid")%>'">좋아요취소</button></th>
				</c:if>
			</tr>
		</c:forEach>
	</table>
