...
public class APIService {
        ...
        for(int eventType = parser.getEventType(); eventType != XmlPullParser.END_DOCUMENT; eventType = parser.next()) {
        	...
        	if(eventType == XmlPullParser.START_TAG && parser.getName().equals("item")) {
        		...
        		if(contenttypeid.equals("32"))
                    value.put("price", nf.format((int) (Math.random() * 1000) * 100));
	...
	//이미지 정보조회
	 	public HashMap<String, String> imgInfo(String contentid, String contenttypeid) throws Exception{
	 		HashMap<String, String> value = new HashMap<>();
	 		
	 	        StringBuilder urlBuilder = new StringBuilder("http://api.visitkorea.or.kr/openapi/service/rest/KorService/detailImage"); /*URL*/
	 	        urlBuilder.append("?" + URLEncoder.encode("ServiceKey","UTF-8") + "=9HZQmtjIkur6iyfPIGo7geVL7zZg%2F4G0sYllDJZH8zTueXqSUtz2Doyj%2BNEAB7Czqo8%2BFub9NIku30J2zVMxFQ%3D%3D"); /*Service Key*/
	 	        urlBuilder.append("&" + URLEncoder.encode("ServiceKey","UTF-8") + "=" + URLEncoder.encode("SERVICE_KEY", "UTF-8")); /*공공데이터포털에서 발급받은 인증키*/
