...
public class APIService {
	...
	public List<HashMap<String, String>> getFestivalInfo() throws Exception {
		...
		urlBuilder
				.append("?" + URLEncoder.encode("ServiceKey", "UTF-8") + "=" + constant.Key.DAUMKEY); /* Service Key */
		 ...
		 * URLEncoder.encode("12", "UTF-8")); /* 관광타입 ID
		 */
			...
			if (eventType == XmlPullParser.START_TAG && parser.getName().equals("item")) {
				...
				for (eventType = parser.next(); eventType != XmlPullParser.END_TAG
					...
					if (eventType == XmlPullParser.START_TAG)
						if (parser.getName().equals("addr1") || parser.getName().equals("title")
								|| parser.getName().equals("mapy") || parser.getName().equals("mapx"))
	...
	// 키워드 검색
	public List<HashMap<String, String>> searchAPIInfo(String search, String contenttypeid, String areaCode)
			throws Exception {
		List<HashMap<String, String>> result = new ArrayList<>();
		...
		urlBuilder
				.append("?" + URLEncoder.encode("ServiceKey", "UTF-8") + "=" + constant.Key.DAUMKEY); /* Service Key */
		urlBuilder.append("&" + URLEncoder.encode("ServiceKey", "UTF-8") + "="
				+ URLEncoder.encode("SERVICE_KEY", "UTF-8")); /* 공공데이터에서 받은 인증키 */
		urlBuilder.append(
				"&" + URLEncoder.encode("MobileApp", "UTF-8") + "=" + URLEncoder.encode("sample", "UTF-8")); /* 서비스명 */
		urlBuilder.append("&" + URLEncoder.encode("MobileOS", "UTF-8") + "="
				+ URLEncoder.encode("ETC", "UTF-8")); /* AND(안드로이드),IOS,ETC(web) */
		urlBuilder
				.append("&" + URLEncoder.encode("pageNo", "UTF-8") + "=" + URLEncoder.encode("1", "UTF-8")); /* 페이지번호 */
		urlBuilder.append(
				"&" + URLEncoder.encode("numOfRows", "UTF-8") + "=" + URLEncoder.encode("15", "UTF-8")); /* 표출건수 */
		urlBuilder.append(
				"&" + URLEncoder.encode("keyword", "UTF-8") + "=" + URLEncoder.encode(search, "UTF-8")); /* 키워드 */
		urlBuilder.append("&" + URLEncoder.encode("areaCode", "UTF-8") + "="
				+ URLEncoder.encode(areaCode, "UTF-8")); /* 지역코드(areaCode 없을때), 시군구코드(areaCode=1) */
		urlBuilder.append("&" + URLEncoder.encode("sigunguCode", "UTF-8") + "="
				+ URLEncoder.encode("", "UTF-8")); /* 지역코드(areaCode 없을때), 시군구코드(areaCode=1) */
		urlBuilder.append("&" + URLEncoder.encode("contentTypeId", "UTF-8") + "="
		...
		urlBuilder.append(
				"&" + URLEncoder.encode("cat2", "UTF-8") + "=" + URLEncoder.encode("", "UTF-8")); /* 중분류코드(대분류코드 필수) */
		urlBuilder.append("&" + URLEncoder.encode("cat3", "UTF-8") + "="
				+ URLEncoder.encode("", "UTF-8")); /* 소분류코드(중분류,대분류코드 필수) */
		urlBuilder.append("&" + URLEncoder.encode("arrange", "UTF-8") + "="
				+ URLEncoder.encode("B", "UTF-8")); /* A=제목순,B=인기순,C=최근수정순,D=등록순 */
		URL url = new URL(urlBuilder.toString());
		HttpURLConnection conn = (HttpURLConnection) url.openConnection();
		conn.setRequestMethod("GET");
		conn.setRequestProperty("Content-type", "application/json");
			...
			if (eventType == XmlPullParser.START_TAG && parser.getName().equals("item")) {
				...
				for (eventType = parser.next(); eventType != XmlPullParser.END_TAG
					...
					if (eventType == XmlPullParser.START_TAG && (parser.getName().equals("title")
							|| parser.getName().equals("addr1") || parser.getName().equals("mapy")
							|| parser.getName().equals("mapx") || parser.getName().equals("contentid")
	...
	public List<HashMap<String, String>> Detail(String contentid, String contenttypeid) throws Exception {
		...
		urlBuilder.append(
				"?" + URLEncoder.encode("ServiceKey", "UTF-8") + "=" + constant.Constant.TOURKEY); /* Service Key */
		urlBuilder.append("&" + URLEncoder.encode("Servicekey", "UTF-8") + "="
				+ URLEncoder.encode("인증키", "UTF-8")); /* 공공데이터포털에서 발급받은 인증키 */
		...
		urlBuilder.append("&" + URLEncoder.encode("contentTypeId", "UTF-8") + "="
				+ URLEncoder.encode(contenttypeid, "UTF-8")); /* 관광타입(관광지, 숙박 등)ID */
		urlBuilder.append(
				"&" + URLEncoder.encode("detailYN", "UTF-8") + "=" + URLEncoder.encode("Y", "UTF-8")); /* 반복정보 조회여부 */
		URL url = new URL(urlBuilder.toString());
		HttpURLConnection conn = (HttpURLConnection) url.openConnection();
		conn.setRequestMethod("GET");
		conn.setRequestProperty("Content-type", "application/json");
			...
			if (eventType == XmlPullParser.START_TAG && parser.getName().equals("item")) {
				for (eventType = parser.next(); eventType != XmlPullParser.END_TAG
					...
					if (eventType == XmlPullParser.START_TAG && (parser.getName().equals("roomtitle")
							|| parser.getName().equals("roomtitle") || parser.getName().equals("roomsize1")
							|| parser.getName().equals("roomcount") || parser.getName().equals("roombasecount")
							...
							|| parser.getName().equals("roompeakseasonminfee2") || parser.getName().equals("roomimg1")
							|| parser.getName().equals("roomimg2") || parser.getName().equals("infoname")
	...
	// 추천하기 리스트 api 연동
	public List<HashMap<String, String>> festival() throws Exception {
		...
		for (int i = 0; festivalList.size() < i; i++) {
			...
			urlBuilder.append("?" + URLEncoder.encode("ServiceKey", "UTF-8")
					+ "=9HZQmtjIkur6iyfPIGo7geVL7zZg%2F4G0sYllDJZH8zTueXqSUtz2Doyj%2BNEAB7Czqo8%2BFub9NIku30J2zVMxFQ%3D%3D"); 
			urlBuilder.append("&" + URLEncoder.encode("ServiceKey", "UTF-8") + "="
					+ URLEncoder.encode("SERVICE_KEY", "UTF-8")); /* 공공데이터포털에서 발급받은 인증키 */
			urlBuilder.append("&" + URLEncoder.encode("numOfRows", "UTF-8") + "="
					+ URLEncoder.encode("10", "UTF-8")); /* 한 페이지 결과 수 */
			urlBuilder.append(
					"&" + URLEncoder.encode("pageNo", "UTF-8") + "=" + URLEncoder.encode("1", "UTF-8")); /* 현재 페이지 번호 */
			...
			urlBuilder.append("&" + URLEncoder.encode("MobileApp", "UTF-8") + "="
					+ URLEncoder.encode("AppTest", "UTF-8")); /* 서비스명=어플명 */
			urlBuilder.append("&" + URLEncoder.encode("contentId", "UTF-8") + "="
					+ URLEncoder.encode(festivalList.get(i).get("contentID"), "UTF-8")); /* 콘텐츠 ID */
			urlBuilder.append("&" + URLEncoder.encode("contentTypeId", "UTF-8") + "="
					+ URLEncoder.encode(festivalList.get(i).get("contentTypeID"), "UTF-8")); /* 관광타입(관광지,숙박 등) ID */
			urlBuilder.append("&" + URLEncoder.encode("imageYN", "UTF-8") + "="
					+ URLEncoder.encode("Y", "UTF-8")); /* Y=콘텐츠 이미지 조회, N='음식점'타입의 음식메뉴 이미지 */
